...
<html>
  <head>
    <meta charset="UTF-8" />
    <title>Delay Discounting Task (jsPsych 8)</title>
    
    <!-- Core jsPsych v8.2.0 -->
    <script src="https://unpkg.com/jspsych@8.2.0/dist/index.browser.min.js"></script>
    
    <!-- Plugins you need -->
    <script src="https://unpkg.com/@jspsych/plugin-html-keyboard-response@2.0.4/dist/index.browser.min.js"></script>
    <script src="https://unpkg.com/@jspsych/plugin-instructions@2.0.4/dist/index.browser.min.js"></script>
    <script src="https://unpkg.com/@jspsych/plugin-survey-text@2.0.4/dist/index.browser.min.js"></script>
    
    <style>
      /* Basic styling for “centerbox”, “center-block-text”, “table”, etc. */
      .centerbox {
        display: flex;
  ...
  <body>
    <script>
      ...
      function evalAttentionChecks() {
        // In modern jsPsych, you might store 
        // attention-check trials with data, then filter.
        // Example placeholder:
        let checks_passed = 0;
      ...
      function assessPerformance() {
        ...
        for(let i=0; i<experiment_data.length; i++){
          if(experiment_data[i].possible_responses !== 'none'){
            ...
            let key = experiment_data[i].response;  // in v8, 'response' is the pressed key
        ...
        if(rt_array.length > 0){
          ...
          avg_rt = (rt_array.length % 2 !== 0) ? rt_array[mid] 
                                              : (rt_array[mid - 1] + rt_array[mid]) / 2;
        }
        let missed_percent = missed_count / trial_count;
        
        // For "credit_var" in the old code
        let credit_var = (missed_percent < 0.4 && avg_rt > 200);
        
        // pick a random bonus from the previously-collected bonus_list
        let bonus = randomDraw(bonus_list);
        
        // Here, you can add data on the last trial or globally:
      ...
      // Box-Muller to get normal random draws
       ...
       *********************************************/
      let run_attention_checks = false;  // can set to true if you have actual attention checks
      let attention_check_thresh = 0.65;
      let sumInstructTime = 0;
      let instructTimeThresh = 0; // e.g., 30 seconds
      let credit_var = true;

      // define possible keys
      let choices = ['q','p'];
      // define bonus list for later random selection
      let bonus_list = [];
      ...
      // combine into an array of trial objects
      let trials = [];
      for (let i = 0; i < 36; i++) {
        trials.push({
          stimulus:
      ...
      const jsPsych = initJsPsych({
        ...
        on_finish: function() {
          // If you *don't* want to do the data sending in a separate block,
          // you could do it here. 
          // e.g., let data_json = jsPsych.data.get().json();
          // fetch(...)

          // Otherwise, the final block below does it.
          console.log("Experiment finished.");
        }
      });

      /*********************************************
       * Instruction & Feedback Blocks
       *********************************************/
      // Instead of poldrack-text, we use html-keyboard-response
      ...
      // Instead of poldrack-instructions, use instructions plugin
      ...
      // We won't replicate the old "loop_function" for reading time. 
      // If you need it, you can place a loop around these instruction trials.
      ...
      // Practice trial (uses html-keyboard-response)
      let practice_block = {
        type: jsPsychHtmlKeyboardResponse,
        stimulus: `
          <div class="centerbox" id="container">
        ...
        on_finish: function(data) {
          // data.response is the key pressed
      ...
      // Build the main test procedure
      // We'll create an array of trial definitions from "trials" above
      let test_block = {
      ...
      // If you have an attention check, you'd define it as a separate html-keyboard-response or similar.
      ...
      // End block, including data sending
      let end_block = {
        type: jsPsychHtmlKeyboardResponse,
        stimulus: `<div class='centerbox'>
      ...
      // If using an attention check:
      // if (run_attention_checks) timeline.push(your_attention_check_block);
